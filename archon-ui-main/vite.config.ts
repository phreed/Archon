...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
  ...
  const isDocker =
    process.env.DOCKER_ENV === "true" || existsSync("/.dockerenv");
  ...
  const port =
    process.env.ARCHON_SERVER_PORT || env.ARCHON_SERVER_PORT || "8181";
  ...
  return {
    ...
    server: {
      ...
      port: parseInt(
        process.env.ARCHON_UI_PORT || env.ARCHON_UI_PORT || "3737",
